...
from ..motor.Instances import Square, EasingDirection
from ..motor.Instances import EasingStyle
...
class Drone():
    ...
    PrecalculatedPaths: Dict[int, List[List[Cell]]] | None = None
    FlyTime: float = 0.1
    ...
    def __init__(self, starting: Cell,
                 target: Cell, Settings: Dict[str, Any],
                 paths: Dict[int, List[List[Cell]]], id: int):
        '''Start the drone'''
    ...
    def Move(self) -> None:
        ...
        if self.DesinationReached:
            return
        try:
            if (self.Next and 1 > self.Moving > 0
                and (self.Next.isOk() or
    ...
    def MoveAlong(self) -> None:
        ...
        oknext = (nextcell.isOk() or nextcell is self.Target if nextcell
                  else False)

        if (nextcell and oknext and nextco.isOk()):
            if self.Next:
                self.Next.Remove(self)
                self.Next = nextcell
                self.Next.Insert(self)
        ...
        elif (nextcell and not oknext and nextcell.Zone
